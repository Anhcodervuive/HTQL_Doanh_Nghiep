...
export default function ListItemPage() {
  ...
  const itemTypes = itemTypeRes?.data?.itemTypes ?? []
  ...
  return (
    <Box sx={{ bgcolor: 'background.default', minHeight: '100vh' }}>
      ...
      <Box sx={{ maxWidth: 1196, mx: 'auto' }}>
        <Paper component="form" sx={{ px: 2, py: 0.5, display: 'flex', alignItems: 'center', border: '1px solid #ccc', borderRadius: 2 }}>
          <InputBase
      ...
      <Box sx={{ px: 4, mt: 3 }}>
        <Grid container columnSpacing={3} rowSpacing={4} sx={{ maxWidth: 1196, mx: 'auto' }}>
          {isLoading ? 'Loading...' : items.map((p) => {
            const basePrice = p.PRICE?.[0]?.PRICE_AMOUNT ?? 0
            ...
            return (
              <Grid item xs="auto" key={p.ITEM_CODE} sx={{ px: 1 }}>
                <Card
                  onClick={() => navigate(`/customer/detail-Item/${p._id}`)}
                  sx={{ cursor: 'pointer', width: 180, height: 300, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', border: '2px solid', borderColor: theme.palette.primary.main, borderRadius: 2, position: 'relative', overflow: 'hidden', boxShadow: 3, bgcolor: theme.palette.background.default }}>
                  {discount > 0 && (
